...
def getflp():
    ...
    vsize = request.args.get('vsize')
    ipipe = request.args.get('ipipe')
    fl = request.args.get('fl')
    val = flP(float(cv), float(vsize), float(ipipe), 890, float(fl))
...
def getfp():
    ...
    vsize = request.args.get('vsize')
    ipipe = request.args.get('ipipe')
    opipe = request.args.get('opipe') 
    val = fP(float(cv), float(vsize), float(ipipe), float(opipe), 890)
    return str(val)
...
def getDH():
    ...
    Iw = request.args.get('Iw')
    val = DH(float(A), float(Iw))
    print(f'DHVALUES {val}')
    return str(val)
...
def getdo():
    ...
    val = DO(float(A), float(No))
    print(f'DOVALUES {val}')
    return str(val)
...
def getfd():
    ...
    val = FD(float(dh), float(do))
    print(f'FDVALUES {val}')
    return str(val)
...
def gasNoise():
    if request.method == 'POST':
        ...
        a = jsonify(data).json
        print(f'SUBMITVALUES {a}')
        inpres, outpres, flp, fp, gamma, rated_cv, valvesize,inpipesize, outpipesize, xt, intemp, Z, W, M, fd, ts,No,A,Iw,Cs,Co,Di,Id= float(a['inpres'][0]), float(a['outpres'][0]), float(a['combflp'][0]), float(a['pipegeo'][0]),float(a['specheat'][0]),float(a['ratedcv'][0]),float(a['valvesize'][0]),float(a['inpipesize'][0]),float(a['outpipesize'][0]),float(a['presdropratio'][0]),float(a['intemp'][0]),float(a['compfact'][0]),float(a['flowrate'][0]),float(a['molmass'][0]),float(a['valvestylemod'][0]),float(a['pipethick'][0]),float(a['holes'][0]),float(a['singleflowA'][0]),float(a['singleflowP'][0]),float(a['soundpipe'][0]),float(a['soundair'][0]),float(a['pipedia'][0]),float(a['outdia'][0])
        X = findX(inpres,outpres)
        alp = alpha(gamma, flp, fp)
        Pvc = findPvc(inpres,outpres,flp,fp)
        Xvcc = findXvcc(gamma)
        Xc = findXC(gamma, flp, fp)
        Xb = findXb(gamma, flp, fp)
        Xce = findXce(gamma, flp, fp)
        regime_no = getRegime(inpres, outpres, gamma, flp, fp)

        inpres_ = meta_convert_P_T_FR_L('P', inpres, 'pa (a)', 'bar (a)',1000)
        outpres_ = meta_convert_P_T_FR_L('P', outpres, 'pa (a)', 'bar (a)', 1000)
        W_ = meta_convert_P_T_FR_L('FR', W, 'kg/s', 'kg/hr', 1000)
        vsize_ = round(meta_convert_P_T_FR_L('L', valvesize, 'inch', 'm',
                                                 1000))
        print(f'VALVESZIE {vsize_}')
        ...
        pi = inletDensity(inpres, M, 8314, intemp)
        print(f'GASINPRESOUTPRES {pi}')
        req_cv = Cv_gas(inpres_, outpres_, gamma, rated_cv, valvesize, inpipesize, outpipesize, xt, intemp, Z, W_, M, 4, N2['inch'])[0]
        print(f'REQUIRED CV {req_cv}')
        jetV = DJ(float(fd), float(req_cv), float(flp), float(fp), 0.0046)

        if regime_no == 'I':
            Wm = WM_I(gamma, inpres, outpres, flp, fp, pi, W)
            Tvc = TVC_I(gamma, inpres, outpres, flp, fp, intemp)
            Cvc = CVC_I(gamma, inpres, outpres, flp, fp, pi)
            mvc = MVC_I(gamma, inpres, outpres, flp, fp)
            eta = eta_aef_I(gamma, inpres, outpres, flp, fp, -3.8)
            wa = WA_I(gamma, inpres, outpres, flp, fp, pi, W, -3.8)
            peakF = peakFreq_fp_I(gamma, inpres, outpres, flp, fp, pi, 0.2, No, A, Iw, req_cv)
            print(f'VALES {Wm},{Tvc},{Cvc},{mvc},{eta},{wa},{peakF}')
        elif regime_no == 'II':
            Tvc = TVC_II(gamma, intemp)
            Cvc = CVC_II(gamma, inpres, pi)
            Wm = WM_II(gamma, inpres, pi, W)
            mvc = MJ_II(gamma, inpres, outpres, flp, fp)
            eta = eta_aef_II(gamma, inpres, outpres, flp, fp, -3.8)
            wa = WA_II(gamma, inpres, outpres, flp, fp, -3.8, pi, W)
            peakF = peakFreq_fp_II(gamma, inpres, outpres, flp, fp, pi, 0.2, No, A, Iw, req_cv)
        elif regime_no == 'III':
            Tvc = TVC_II(gamma, intemp)
            Cvc = CVC_II(gamma, inpres, pi)
            Wm = WM_II(gamma, inpres, pi, W)
            mvc = MJ_II(gamma, inpres, outpres, flp, fp)
            eta = eta_aef_III(gamma, inpres, outpres, flp, fp, -3.8)
            wa = WA_III(gamma, inpres, outpres, flp, fp, -3.8, pi, W)
            peakF = peakFreq_fp_III(gamma, inpres, outpres, flp, fp, pi, 0.2, No, A, Iw, req_cv)
        elif regime_no == 'IV' or regime_no == 'V':
            Tvc = TVC_II(gamma, intemp)
            Cvc = CVC_II(gamma, inpres, pi)
            Wm = WM_II(gamma, inpres, pi, W)
            mvc = MJ_II(gamma, inpres, outpres, flp, fp)
            eta = eta_aef_IV(gamma, inpres, outpres, flp, fp, -3.8)
            wa = WA_IV(gamma, inpres, outpres, flp, fp, -3.8, pi, W)
            peakF = peakFreq_fp_IV(gamma, inpres, outpres, flp, fp, pi, 0.2, No, A, Iw, req_cv)

        p2 = outDense(inpres, outpres, pi)
        c2 = C2(gamma, 8314, intemp, M)
        mo = MO(gamma, inpres, outpres, pi, 8314, intemp, M, W, Id)
        lg = LG(gamma, inpres, outpres, pi, 8314, intemp, M, W, Di) #Id to outpipesize
        m2 = M2(gamma, inpres, outpres, pi, 8314, intemp, M, W, Di)
        lpi = LPI(gamma, inpres, outpres, flp, fp, pi, W, -3.8, 8314, intemp, M, Id, Di) #Id to outpipesize
        rF = fr(Cs, Di)
        oF = fo(gamma, 8314, intemp, M, Cs, Di, Co)
        gF = fg(Co, Cs, ts)
        print(f'MO_FINALVALUE {mo}')
        aweight = lpae_1m(gamma, inpres, outpres, flp, fp, pi, W, -3.8, 8314, intemp, M, Id,
            Di, 0.2, No, A, Iw, req_cv, Cs, Co, valvesize, ts, 1, 101325, 101325, 8000, -3.8)
        
        print(f'aweight {aweight}')
        ...
        # intemp_ = meta_convert_P_T_FR_L('T', intemp, 'C', 'K',  1000)


        

        intermediate_results = {'X':X, 'Pvc':Pvc, 'Xvcc':Xvcc, 'Xc':Xc, 'Xb':Xb, 'Xce':Xce, 'alpha':alp, 'req_cv':req_cv, 'jetV': jetV, 'regime':regime,'Cvc':Cvc, 'Wm':Wm, 'mvc':mvc, 'eta':eta, 'wa':wa, 'peakF':peakF,'Tvc':Tvc,'p2':p2,'c2':c2,'mo':mo,'lg':lg,'lpi':lpi,'m2':m2,'rF':rF,'oF':oF,'gF':gF,'aweight':aweight}
        print(f'INTERRESULT {intermediate_results}')
        session['intermediate_results'] = intermediate_results

        return redirect(url_for('gasNoise',res=intermediate_results))
        
      
    intermediate_results = session.pop('intermediate_results', {'X': '', 'Pvc': '', 'Xvcc': '', 'Xc': '', 'Xb': '', 'Xce': '', 'alpha':'', 'req_cv':'', 'jetV': '', 'regime':'','Cvc':'', 'Wm':'', 'mvc':'', 'eta':'', 'wa':'', 'peakF':'','Tvc':'','p2':'','c2':'','mo':'','lg':'','lpi':'','m2':'','rF':'','oF':'','gF':''})
    return render_template('gasnoise.html',res=intermediate_results)

@app.template_filter('checkflowChar') 
def checkflowChar(valveType,flowChars):
    flow_chars = []
    butterfly = ['Butterfly Lugged Wafer','Butterfly Double Flanged']
    for flow_char in flowChars:
        if valveType in butterfly:
            if flow_char.name == 'Modified Equal %':
                flow_chars.append(flow_char)
        else:
            flow_chars.append(flow_char)    
...
def checkflowDirection(valveType,flowDirections):
    flow_dirs = []
    butterfly = ['Butterfly Lugged Wafer','Butterfly Double Flanged']
    for flow in flowDirections:
        if valveType in butterfly:
            if not (flow.name == 'Over' or flow.name == 'Under'):
                flow_dirs.append(flow)
        else:
            if flow.name == 'Over' or flow.name == 'Under':
                flow_dirs.append(flow)
    
    return flow_dirs


@app.template_filter('checkValvetype') 
def checkValvetype(valveType,trimdatas):
    trim_data = []
    butterfly = ['Butterfly Lugged Wafer','Butterfly Double Flanged']
    for trim in trimdatas:
        if valveType in butterfly:
            if trim.name == 'Double Offset' or trim.name == 'Triple Offset':
                trim_data.append(trim)
        else:
            if not (trim.name == 'Double Offset' or trim.name == 'Triple Offset'):
                trim_data.append(trim)
    print(f'checkappfilters {trim_data}')
    return trim_data    
...
### ----------------------------------------- Actuator Functions --------------------------------#
def valveForces(p1_, p2_, d1, d2, d3, ua, trimtype, balance, flow,
...
def cv_upload(data_list):
    ...
    # filtered_data = [item for item in new_data_list if item['style'] == 'Globe straight' and item['trimType_'] == 'Ported']
    print(f'CV NAMES ,{new_data_list}')
...
def fP(C, valveDia, inletDia, outletDia, N2_value):
    print('FP VALUES')
    print(C, valveDia, inletDia, outletDia, N2_value)
...
def flP(C, valveDia, inletDia, N2_value, Fl):
    print(f'FLp VALUEs')
    print(C, valveDia, inletDia, N2_value, Fl)
...
def preferences(proj_id, item_id, page):
    ...
    with app.app_context():
        return render_template('preferences.html', user=current_user, metadata=metadata_, item=getDBElementWithId(itemMaster, item_id), page=page,act_case = act_case)
...
def addCompany(proj_id, item_id):
    ...
    if request.method == "POST":
        if request.form.get('addCompany'):
            ...
            db.session.commit()
            flash('Address Edited successfully','error')
            return redirect(url_for('addCompany', item_id=item_id, proj_id=proj_id))
...
def power_level_liquid(flowrate, fl_unit, inletp, inletp_unit, vaporp, vaporp_unit, specificG, xt_fl):

    inletP_ = meta_convert_P_T_FR_L('P', inletp, inletp_unit, 'pa (a)', 1000)
    vaporP_ = meta_convert_P_T_FR_L('P', vaporp, vaporp_unit, 'pa (a)', 1000)
    flowrate_ = meta_convert_P_T_FR_L('FR', flowrate, fl_unit, 'kg/s', 1000)
    pL = specificG * 1000
    delpc = (xt_fl * xt_fl) * (inletP_ - vaporP_)
    jet_vel = (1 / xt_fl) * (math.sqrt((2 * delpc)/pL))
    p_level = (flowrate_ * (jet_vel * jet_vel) * (xt_fl * xt_fl)) / 2
    print(f'P_LEVEL_LIQUID {p_level}')
    return p_level
...
def unit_change():
    ...
    print(f'unitchange_param_values{prev_unit},{final_unit},{params},{specific_gravity},{fluid_type}, {param_values}')
    ...
    pressure_params = ['setPressureUnit','lower_benchsetUnit','upper_benchsetUnit','inpres','outpres','vaporPres','critPres','shutoffPres','maxPres','ipres','opres','delpflow','lowBenchSet','UpBenchSet','maxAirSupply','setPressure','frictionBand','setPUnit','maxAirUnit','delp','presdrop']
...
def getOutputs(flowrate_form, fl_unit_form, inletPressure_form, iPresUnit_form, outletPressure_form, oPresUnit_form,
    ...
    try:
        ...
        flash('Vapour pressure should not be greater than InletPressure','error')
        pLevel = 1
...
def getOutputsGas(flowrate_form, fl_unit_form, inletPressure_form, iPresUnit_form, outletPressure_form, oPresUnit_form,
    ...
    print(f'INSIDE OUTPUTSGAS FLPFP {flp}, {fp}')
...
# print(f"pipe dia: {inletPipeDia_v}, sch: {iSch}")
    ...
    # pLevel = power_level_liquid(inletPressure_p, outletPressure_p, specificGravity, result)
    pLevel = power_level_liquid(flowrate_form, fl_unit_form, inletPressure_form, iPresUnit_form, vaporPressure, vPresUnit_form, specificGravity, xt_fl)
...
def gasSizing(inletPressure_form, outletPressure_form, inletPipeDia_form, outletPipeDia_form, valveSize_form,
    ...
    result_dict = {
        ...
        'sonicVelValve': mac_sonic_list[5],
        'outletDensity': mac_sonic_list[6],
        'x_delp': round((inletPressure_form-outletPressure_form)/inletPressure_form,3),
        'outletPipeSize': outletPipeDia_form,
...
def getunbalforce():
    ...
    print('kkkkkkjdhdhdd',stemDia_element, len(''.join(str(stemDia_element.stemDia))))
    if len(''.join(str(stemDia_element.stemDia)).strip()) == 1:
        a3 = float(stemDia_element.stemDia)
    elif len(''.join(str(stemDia_element.stemDia)).strip()) == 3:
...
def getseatload():
    ...
    trimtype = request.args.get('trimType')
    valveElement = db.session.query(valveDetailsMaster).filter_by(itemId=itemId).first()
    if valveElement.trimTypeId and valveElement.seatLeakageClassId:
        ...
        trim = getDBElementWithId(trimType,trimtype)
        leakage = db.session.query(seatLeakageClass).filter_by(name='ANSI Class IV').first() 
        seatload = db.session.query(seatLoadForce)\
        .filter_by(trimType_=trim,leakage=leakage)\
        .order_by(func.abs(seatLoadForce.seatBore - float(seatDia)))\
        .first()
        print(f'SEATLOADFACTOR {seatload}')

        

    print('seatloadfactor',trim)
...
def getpackingfriction():
    ...
    if len(''.join(str(stemDia_element.stemDia)).strip()) == 1:
        decimal_stemdia = float(stemDia_element.stemDia)
    elif len(''.join(str(stemDia_element.stemDia)).strip()) == 3:
    ...
    itemId = request.args.get('itemId')
    valveElement = db.session.query(valveDetailsMaster).filter_by(itemId=itemId).first()
    if valveElement.ratingId and valveElement.packingId:
        rating_ = getDBElementWithId(ratingMaster,valveElement.ratingId)
        packing_ = getDBElementWithId(packing,valveElement.packingId)
    else:
        rating_ = db.session.query(ratingMaster).filter_by(name='ASME 600').first()
        packing_ = db.session.query(packing).filter_by(name='PTFE Chevron').first()
        print(f'packingDJDJF {rating_},{packing_}')
...
def valveSizing(proj_id, item_id):
                        ...
                        if a['flowrate'][k]:
                            try:
                                ...
                                new_case = caseMaster(flowrate=output['flowrate'], inletPressure=output['inletPressure'],
                                                        ...
                                                        oSch=oSch,ipipeStatus=a['ipipeStatus'][0],opipeStatus=a['opipeStatus'][0],pressuredrop=a['pressureDrop'][0]
                                                        )
                                ...
                                item_selected.presdrop_unit = a['presdrop_unit'][0]
                        ...
                        if a['flowrate'][k]:
                            try: 
                                ...
                                new_case = caseMaster(flowrate=output['flowrate'], inletPressure=output['inletPressure'],
                                                        ...
                                                        item=item_selected, specificHeatRatio=a['specificHeatRatio'][0], compressibility=a['compressibility'][0], iSch=iSch, oSch=oSch,ipipeStatus=a['ipipeStatus'][0],opipeStatus=a['opipeStatus'][0],pressuredrop=a['pressureDrop'][0])
    ...
    return render_template(html_page, item=getDBElementWithId(itemMaster, int(item_id)), user=current_user,
                           metadata=metadata_, page='valveSizing', valve=valve_element, case_length=range(5),length=5, 
                           cases=itemCases_1, total_length=len(itemCases_1), constants=constants)


@app.route('/itemCaseDelete', methods=['GET','POST'])
def itemCaseDelete():
    proj_id = request.args.get('proj_id')
    item_id = request.args.get('item_id')
    case_id = request.args.get('case_id')
    print(f'ITEMCASEDELETECALLEDssss',{case_id},{proj_id},{item_id})
    db.session.query()
...
def projectDelete():
    ...
    if total_no_of_projects == 1:
        ...
        proj1_id = all_projects[0].id
        item1_id = new_item.id
...
def selectValve(proj_id, item_id):
    ...
    try:
        ...
        flash('Add valve data',"error")
        return redirect(url_for('valveData', item_id=item_id, proj_id=proj_id))
    cases = db.session.query(caseMaster).filter_by(item=item_selected).all()
    if request.method == "POST":
        if len(cases) > 0:
            if request.form.get('getv'):
                print(f'posts {cases[0].fluid.id}')
                cv_all = cvValues.query.all()
                print(f'cv_all ,{cv_all[::4]}')
                 
                db.session.commit()
                print(f'LENGTH ,{len(cv_all)}')
                data = request.form.to_dict(flat=False)
                a = jsonify(data).json
                print(f'akskks',{a['rating'][0]})
               
         
                vType = db.session.query(valveStyle).filter_by(name=a['style'][0]).first()
                ...
                # update changes in valve details
                    ...
                    valve_element.flowCharacter__ = flowChara
                    ...
                    print(f'The final return globe is__: {len(return_globe_data)}, {cases[0].fluid.id}')   
                    ...
                    flash('No CV available to get valves','error')
                ...
                for case_ in cases_new:
                    print(f'getting cases {cases_new[0].fluid}')
                    print(case_.id)
                    if case_.calculatedCv:
                        ...
                        if item_selected.outpipe_unit == "mm":
                            ...
                            o_pipearea_element = db.session.query(pipeArea)\
                                ...
                                .first()
                        if i_pipearea_element and o_pipearea_element:
                        ...
                        if valve_element.state.name == 'Liquid':
                            ...
                            new_case = caseMaster(flowrate=output['flowrate'], inletPressure=output['inletPressure'],
                                        ...
                                        oSch=last_case.oSch,seatDia=valve_d_id.seatBore,ipipeStatus=last_case.ipipeStatus,opipeStatus=last_case.opipeStatus,pressuredrop=last_case.pressuredrop)
            ...
            flash('Add Case to select valve','error')
        else:
...
def actuatorSizing(proj_id, item_id):
    ...
    if request.method == 'POST':
        ...
        actuator_input_dict['availableAirSupplyMinUnit'] = [request.form.get('availableAirSupplyMinUnit')]
        ...
        actuator_input_dict['shutoffDelP'] = [request.form.get('shutoffDelP')]
        actuator_input_dict['shutoffDelPUnit'] = [request.form.get('shutoffDelPUnit')]
...
def getUaElement():
    ...
    itemId = request.args.get('itemId')
    trimtype = request.args.get('trimtype') 
    print(f'INPUTUAELEMENT {trimtype}')
    item_element = getDBElementWithId(itemMaster,itemId)
    valve_element = db.session.query(valveDetailsMaster).filter_by(item=item_element).first()
    if valve_element.trimType__ and valve_element.seatLeakageClass__:
        ...
        trimtype_ = getDBElementWithId(trimType, trimtype)
        leakage_ = db.session.query(seatLeakageClass).filter_by(name='ANSI Class IV').first()
        print(f'UAELE {trimtype},{leakage_}')
        ua_element = db.session.query(unbalanceAreaTb).filter_by(trimType_=trimtype_,seatLeakageClass__=leakage_)\
        .order_by(func.abs(unbalanceAreaTb.seatDia - seatDia))\
        .first()


    print(f'getUAPLUG {ua_element}')
    data = {
...
def compareForces(p1, p2, d1, d2, d3, ua, trimtype, balance, flow,
                  case, shutoffDelP, size, stroke, r1, r2, setPressure, failAction, valveTravel, flowChar, act_type,packingF,seatF,kn_value):
    ...
    vForce_ = valveForces(p1, p2, d1, d2, d3, ua, trimtype, balance, flow,
                          case, shutoffDelP,packingF,seatF)
    vForce_shutoff_ = valveForces(p1, p2, d1, d2, d3, ua, trimtype, balance, flow,
                                  'shutoff', shutoffDelP,packingF,seatF)
                                  
    vForce_open_ = valveForces(p1, p2, d1, d2, d3, ua, trimtype, balance, flow,
                               'open', shutoffDelP,packingF,seatF)
    vForce_close_ = valveForces(p1, p2, d1, d2, d3, ua, trimtype, balance, flow,
    ...
    result_list = [result, springrate, sfMax, sfMin, natMax, natMin, B, C, vForce, vForce_shutoff_, vForce_close_,
                vForce_open_, comment1, comment2, comment3, kn, seatF]
...
def slidingStem(proj_id, item_id):
    ...
    if not cases:
        ...
        try:
            ...
            balancing_element = db.session.query(balancing).filter_by(name=valve_element.balancing__.name).first() 
            print(f'SLIDING BALANCING {balancing_element}')

            fl_ch_element = db.session.query(flowCharacter).filter_by(name=valve_element.flowCharacter__.name).first() 
        except:
            cv_element = None
            selected_sized_valve_element = None
            balancing_element = None
    ...
    if request.method == 'POST':
        ...
        if request.form.get('sliding'):
            ...
            if len(''.join(str(stemsize_.stemDia)).strip()) == 1:
                stem_fraction = float(stemsize_.stemDia)
            elif len(''.join(str(stemsize_.stemDia)).strip()) == 3:
            ...
            act_case_data_json['unbalanceForce'] = a['unbalForce']     
            ...
            act_case_data_json['flowCharacterId'] = a["flowCharacter"]
            ...
            flowCharacter__ = getDBElementWithId(flowCharacter,a["flowCharacter"][0])
            ...
            # Calculations
            valve_forces = valveForces(iPressure, oPressure, seatDia, plugDia, stem_fraction, ua, 
                                     trimType__, balancing__, flowDirection__, 'shutoff+', shutOffDelP,packingF,a['seatF'][0])
            vf_shutoff = valveForces(iPressure, oPressure, seatDia, plugDia, stem_fraction, ua,
                                    trimType__, balancing__, flowDirection__, 'shutoff', shutOffDelP,packingF,a['seatF'][0])
            vf_open = valveForces(iPressure, oPressure, seatDia, plugDia, stem_fraction, ua, 
                                    trimType__, balancing__, flowDirection__, 'open', shutOffDelP,packingF,a['seatF'][0])
            vf_close = valveForces(iPressure, oPressure, seatDia, plugDia, stem_fraction, ua,
                                    trimType__, balancing__,flowDirection__, 'close', shutOffDelP,packingF,a['seatF'][0])
            ...
            print(f'FLOWCHARACTERVALVE {flowCharacter__}')
            ...
            if balancing__.name == 'Unbalanced':
                ...
                kn_element = db.session.query(knValue)\
                    .filter_by(trimType_=trimType__, flowDirection_=flowDirection__, flowCharacter_=flowCharacter__)\
            ...
            act_case_data_json_result['shutOffForce'] = [round(valve_forces[3],2)]
            ...
            setPressure = meta_convert_P_T_FR_L('P', setPressure, setPressureUnit,
                                        'psia (g)', 1.0 * 1000)
            if a['valveThrustClose'][0] =='' and a['valveThrustOpen'][0] =='' and a['shutOffForce'][0] == '':
                flash('Calculate actuator before selecting Actuator','error')
                return redirect(url_for('slidingStem', item_id=item_id, proj_id=proj_id))     
            ...
            act_fun_output = compareForces(iPressure, oPressure, seatDia, plugDia,
                stem_fraction, ua, act_case_data.trimType_, act_case_data.balancing_,act_case_data.flowDirection_ ,
                ...
                act_case_data.valveTravel, act_case_data.flowCharacter_, act_element_sliding.actType,act_case_data.packingFriction,act_case_data.seatloadFactor,act_case_data.knValue) 
            ...
            if hw_thrust:
                ...
                maxHW = ''
            airsupplyMin =  meta_convert_P_T_FR_L('P', float(act_master.availableAirSupplyMin), act_master.availableAirSupplyMinUnit,
                                        'psia (g)', 1.0 * 1000)
            airsupplyMax = meta_convert_P_T_FR_L('P', float(act_master.availableAirSupplyMax), act_master.availableAirSupplyMaxUnit,
                                        'psia (g)', 1.0 * 1000)
            ...
            if valvecloseunit_start_index is not None:
                ...
                for i, (key, value) in enumerate(data.items()):
                    if i >= valvecloseunit_start_index:
                        # Convert specific keys to float
                        if key in ['valveThrustClose', 'valveThrustOpen','shutOffForce','stemArea','negGrad','act_travel','diaphragm_ea','lower_benchset','upper_benchset',
                                   'spring_rate','springWindUp','maxSpringLoad','airsupply_min','airsupply_max','sfMin','natMin','frictionBand']:  
                            results_[key] = float(value[0])
                        else:
                            results_[key] = value[0]
            results_.pop('saveop')
            print(f'CALC RESULTS ACTSD {results_}')
            act_case_data.update(results_,act_case_data.id)
            db.session.commit()
            return redirect(url_for('slidingStem', item_id=item_id, proj_id=proj_id))
    ...
    return render_template('slidingstem.html', item=getDBElementWithId(itemMaster, int(item_id)), 
                           ...
                           cases=cases,trimtType=trimType_element,fl_d_element=fl_d_element, fl_ch_element=fl_ch_element,
...
def rotaryActuator(proj_id, item_id):
    ...
    if not cases:
        ...
        cv_element = db.session.query(cvValues).filter_by(cv=cases[0].cv).first()
        shaftdia_ = db.session.query(shaftRotary).filter_by(rating=valve_element.rating,valveSize=cases[0].valveSize).first()
        print(f'ACTCASEDATA {shaftdia_.stemDia}')
        if len(''.join(str(shaftdia_.stemDia)).strip()) == 1:
            a3 = float(shaftdia_.stemDia)
        elif len(''.join(str(shaftdia_.stemDia)).strip()) == 3:
            a3 = float(Fraction(''.join(str(shaftdia_.stemDia))))

        else:
            integer_part, fractional_part = str(shaftdia_.stemDia).split(' ')
            numerator, denominator = map(int, fractional_part.split('/'))

            # Perform the addition
            a3 = float(integer_part) + numerator / denominator
        print(f'SHAFTDUA {a3},{shaftdia_}')
    ...
    if request.method == 'POST':
        ...
        if request.form.get('sliding'):
            ...
            data.pop('availableAirSupplyMinUnit')
            ...
            if valve_element.shaftId:
                shaft_mat = getDBElementWithId(shaft,valve_element.shaftId)
            else:
                print(f'SHAFTMATERIAL {a['shaftId'][0]}')
                shaft_mat = getDBElementWithId(shaft,a['shaftId'][0])
            ...
            if not act_case_data.bto:
                flash("Calculate Actuator before Selecting","error")
                return redirect(url_for('rotaryActuator', item_id=item_id, proj_id=proj_id))
            print(f'SETBEFOREPRESSURE {act_element.availableAirSupplyMax}, {act_element.availableAirSupplyMaxUnit}')
    ...
    return render_template('RotaryActuatorSizing.html', item=getDBElementWithId(itemMaster, int(item_id)), 
                         ...
                         valve=valve_element, act=act_element,act_case_data=act_case_data,cases=cases,cv_element=cv_element,v_style=v_style,shaft_dia=a3)
...
def strokeTime(proj_id, item_id):
    ...
    if request.method == "POST":
        ...
        if request.form.get('strokeinput'):
            ...
            packingFriction_ =  float(data['packing_friction']) 
        ...
        if request.form.get('interstroke'):
            ...
            stroke_element.update(a_, stroke_element.id)
            flash('Stroke Time calculation completed')
        ...
        if request.form.get('clear'):
            ...
            if stroke_element.piExhaust or stroke_element.prefillTime:
                ...
                flash('Calculate before deleting','error')
                        ...
                        if len(cases) == 0:
                            ...
                            for i in itemCases_1[:6]:
                                ...
                                if v_details.state.name == 'Gas':
                                    ...
                                    ind_5,ind_6,ind_7,ind_8 = i.specificGravity,i.kinematicViscosity, i.vaporPressure,i.fl
                                case_list = [i.flowrate, i.inletPressure, i.outletPressure, i.inletTemp, ind_5, ind_6, ind_7, ind_8,
                            ...
                            if v_details.valveSeries in ['10','11']:    
                                ...
                                if act_mas.actuatorType:
                                    act_modelno = getActModelNo(act_mas.actuatorType,act_case.act_size,act_mas.springAction,act_mas.handWheel,act_mas.adjustableTravelStop)
                                else:
                                    act_modelno = 'N/A'
                            else:
                                act_case = db.session.query(rotaryCaseData).filter_by(actuator_=act_mas).first()
                                if act_mas.actuatorType:
                                    act_modelno = getActModelNo(act_mas.actuatorType,act_case.actSize_,act_mas.springAction,act_mas.handWheel,act_mas.adjustableTravelStop)
                                else:
                                    act_modelno = 'N/A'
                    ...
                    for item in items:
                        ...
                        if v_type == 'globe':
                            ...
                            units = [
                                ...
                                act_case.valveSizeUnit,
                                act_case.discDiaUnit,
                                act_case.shaftDiaUnit,
                                act_case.delpUnit,
                                '',
                                '',
                                '',
                                ...
                                '',
                                '',
                                act_case.stStartUnit,
                                act_case.stMidUnit,
                                act_case.stEndUnit,
                                act_case.atStartUnit,
                                act_case.atMidUnit,
                                act_case.atEndUnit,
                                '',
                                ''
...
def getActModelNo(series, size, faction, manualOverride, limitStop):
    ...
    size_ = {'1A': 'S38', '1B': 'S75', '1C': 'S150', '1D': 'S300', '2A': 'M60', '2B': 'M100', '2C': 'M160', '3A': 'P30', '3B': 'P50', '3C': 'P110', '4A': 'P110L', '5A': 'P110D', '5B': 'P220D'}
    faction_ = {'1': 'AFO', '2': 'AFC', '3': 'DA'}
            ...
            for report in report_list:
                ...
                if report == 'controlvalve':
                    ...
                    for item in all_items:
                        v_details = db.session.query(valveDetailsMaster).filter_by(item=item).first()
                        acc_details = db.session.query(accessoriesData).filter_by(item=item).first()
                        acc_list = [acc_details.manufacturer if acc_details.manufacturer else 'N/A', 'See N2' if acc_details.model else 'N/A', acc_details.action if acc_details.action else 'N/A', 
                                    str(acc_details.afr).split('/')[0] if acc_details.afr else 'N/A',
                                    'See N3' if acc_details.afr else 'N/A',
                                    acc_details.transmitter if acc_details.transmitter else 'N/A' , 'See N4' if acc_details.limit else 'N/A' , acc_details.proximity if acc_details.proximity else 'N/A', 
                                    ...
                                    'See N5' if acc_details.solenoid_model else 'N/A',
                        ...
                        if len(cases) == 0:
                            ...
                            try:
                                ...
                                act_modelno = 'N/A';open_time = 'N/A'; close_time = 'N/A'
                            act_dict_ = {
                                    'act_type':  act_mas.actuatorType if act_mas.actuatorType else 'N/A',
                                    'set_pressure': f"{act_mas.availableAirSupplyMax} {act_mas.setPressureUnit}",
                                    'air_supply_pressure': f"{act_mas.availableAirSupplyMin} {act_mas.availableAirSupplyMaxUnit}",
                                    'fail_action': f"{act_mas.springAction}" if act_mas.springAction else 'N/A',
                                    'orientation': f"{act_mas.orientation}" if act_mas.orientation else 'N/A',
                                    'act_travel': (f"{act_case.max_rot}" if v_details.valveSeries in ['20','21'] else f"{act_case.act_travel} {act_case.actuatorTravelUnit}" ) if act_case else 'N/A',
                                    'spring':f"{act_case.lower_benchset} - {act_case.upper_benchset}" if act_case and v_details.valveSeries in ['10','11'] else 'N/A' ,
                                    'act_size':(act_case.act_size if v_details.valveSeries in ['10','11'] else act_case.actSize_) if act_case else 'N/A',
                                    'cleaning':access_.cleaning if access_.cleaning else 'N/A' ,
                                    'paint_finish':access_.paint_finish if access_.paint_finish else 'N/A',
                                    'product_certs':access_.paint_cert if access_.paint_cert else 'N/A',
                    ...
                    header_details = []
                    # enquiry_received_dt =type(str(project_element.enquiryReceivedDate))
                    # print(f'DATE {enquiry_received_dt}')
                    ...
                    for item in items:
                        ...
                        if v_type == 'globe':
                            valvedata = [
                                    ...
                                    act_case.stemDia if act_case.stemDia else 'N/A',
                                    act_case.valveTravel if act_case.valveTravel else 'N/A',
                                    act_case.packingFriction if act_case.packingFriction else 'N/A',
                                    act_case.seatloadFactor if act_case.seatloadFactor else 'N/A',
                                    'N/A',
                                    act_case.iPressure if act_case.iPressure else 'N/A',
                                    act_case.oPressure if act_case.oPressure else 'N/A',
                                    act_case.shutOffDelP if act_case.shutOffDelP else 'N/A',
                                    act_case.unbalForce if act_case.unbalForce else 'N/A',
                                    act_case.negGrad if act_case.negGrad else 'N/A',
                                    act_case.iPressure - act_case.oPressure if act_case.iPressure else 'N/A',
                                    ''
                            ]
                            actdata = [ 
                                act_mas.actuatorType if act_mas.actuatorType else 'N/A',
                                act_mas.springAction if act_mas.springAction else 'N/A',
                                act_case.act_size if act_case.act_size else 'N/A',
                                act_case.act_travel if act_case.act_travel else 'N/A',
                                act_case.diaphragm_ea if act_case.diaphragm_ea else 'N/A',
                                act_case.lower_benchset if act_case.lower_benchset else 'N/A',
                                act_case.upper_benchset if act_case.upper_benchset else 'N/A',
                                act_case.spring_rate if act_case.spring_rate else 'N/A',
                                act_case.springWindUp if act_case.springWindUp else 'N/A',
                                act_case.maxSpringLoad if act_case.maxSpringLoad else 'N/A',
                                act_case.airsupply_max if act_case.airsupply_max else 'N/A',
                                act_case.airsupply_min if act_case.airsupply_min else 'N/A',
                                act_case.frictionBand if act_case.frictionBand else 'N/A',
                                'N/A',
                                'N/A',
                            ...
                            va_forces = [
                                act_case.valveThrustClose if act_case.valveThrustClose else 'N/A',
                                act_case.valveThrustOpen if act_case.valveThrustOpen else 'N/A',
                                act_case.shutOffForce if act_case.shutOffForce else 'N/A',
                                '',
                                '',
                                '',
                                actThrustClose if actThrustClose else 'N/A',
                            ...
                            valvedata = [
                                ...
                                act_case.shaft_dia if act_case.shaft_dia else 'N/A',
                                ...
                                act_case.pt if act_case.pt else 'N/A',
                                act_case.ft if act_case.ft else 'N/A'

                            ]
                            actdata = [ 
                                act_mas.actuatorType,
                                act_mas.springAction,
                                act_case.actSize_ if act_case.actSize_ else 'N/A',
                                act_case.max_rot if act_case.max_rot else 'N/A',
                                act_case.actSize_ if act_case.actSize_ else 'N/A',
                                act_case.springSet if act_case.springSet else 'N/A',
                                act_case.maxAir if act_case.maxAir else 'N/A',
                                act_case.setP if act_case.setP else 'N/A',
                                'N/A',
                                'N/A',
                                '',
                                '',
                            ...
                            va_forces = [
                                act_case.bto if act_case.bto else 'N/A',
                                act_case.rto if act_case.rto else 'N/A', 
                                act_case.eto if act_case.eto else 'N/A',
                                act_case.btc if act_case.btc else 'N/A',
                                act_case.rtc if act_case.rtc else 'N/A',
                                act_case.etc if act_case.etc else 'N/A',
                                act_case.springSt if act_case.springSt else 'N/A',
                                act_case.springMd if act_case.springMd else 'N/A',
                                act_case.springEd if act_case.springEd else 'N/A',
                                act_case.AirSt if act_case.AirSt else 'N/A',
                                act_case.AirMd if act_case.AirMd else 'N/A',
                                act_case.AirEd if act_case.AirEd else 'N/A',
                                'N/A',
                                'N/A'
                            ]
                            units = [
...
def exportProject(item_id, proj_id):
    ...
    all_items = db.session.query(itemMaster).filter_by(project=project_element).all()
    heading = [['Project'],['Items'],['CaseDetails'],['Actuator'],['Accessories']]
    ...
    item_id = 1; case_id = 1; act_id = 1;acc_id = 1
    ...
    for data_ in all_items:
        ...
        for key_ in item_datas:
            abc = getattr(data_, key_)
            print(f'ITEMSDATA {abc}')
            if abc:
                if key_ == 'id':
                    single_row_data.append(item_id)
                else:
                    single_row_data.append(abc)
            else:
                single_row_data.append('N/A')
        ...
        keyTable = { 'ratingId': ratingMaster, 'materialId': materialMaster, 'designStandardId': designStandard,
                'valveStyleId': valveStyle, 'fluidStateId': fluidState, 'fluidPropertiesId': fluidProperties, 'endConnectionId': endConnection, 'endFinishId': endFinish,
                'bonnetTypeId': bonnetType, 'packingTypeId': packingType, 'trimTypeId': trimType, 'flowCharacterId': flowCharacter, 'flowDirectionId': flowDirection, 
                'seatLeakageClassId': seatLeakageClass, 'bonnetId': bonnet, 'nde1Id': nde1, 'nde2Id': nde2, 'shaftId': shaft, 'discId': disc, 'seatId': seat, 'packingId': packing, 
                'balancingId': balancing, 'balanceSealId': balanceSeal, 'studNutId':studNut , 'gasketId': gasket, 'cageId': cageClamp
                }
        def get_name_from_master(field, iD):
            if field in keyTable:
                master_table = keyTable[field]
                abc_name = getDBElementWithId(master_table, int(iD))
                return abc_name.name
            return None
        for key_ in valve_datas:
            abc = getattr(valve_item, key_)
            if abc:
                if key_ in keyTable:
                    single_row_data_valve.append(get_name_from_master(key_, abc))
                else:
        ...
        cases_ = db.session.query(caseMaster).filter_by(item=data_).all()
            ...
            for key_ in all_keys_cases:
                ...
                if abc:
                    if key_ == 'id':
                        ...
                        single_row_data_case.append(item_id)
        ...
        actuator_mas = db.session.query(actuatorMaster).filter_by(item=data_).all()
        for act_ in actuator_mas:
            ...
            for key_ in all_keys_actuator:
                ...
                if abc:
                    if key_ == 'id':
                        ...
                        single_row_data_act.append(item_id)
                    elif key_ not in ['valveDiaId', 'fluidId']:
            ...
            if act_.actuatorType in ['SY','SYC','SYCDA']:
                actuator_case = db.session.query(rotaryCaseData).filter_by(actuator_=act_).first()
                ...
                actuator_case = db.session.query(actuatorCaseData).filter_by(actuator_=act_).first()
                for key_ in all_keys_slidingCase:
                    abc = getattr(actuator_case, key_)
                    if abc:
                        single_row_data_act.append(abc)
            ...
            accessories_data = db.session.query(accessoriesData).filter_by(item=data_).all()  
            for acc_ in accessories_data:
                ...
                for key_ in all_keys_accessories:
                    ...
                    if abc:
                        if key_ == 'id':
                            ...
                            single_row_data_accessories.append(item_id)
                        else:
    ...
    item_details.insert(0, allkeys_item) #key
    for item_detail in item_details: #values
        ws.append(item_detail)

    ws.append([])
    # Appending the case details heading
    ws.append(heading[2])
    # Appending the case details to the worksheet
    case_details.insert(0, all_keys_cases) # key
    for case_detail in case_details: #values
    ...
    act_details.insert(0, all_keys_actdatas) # key
    for act_detail in act_details: #values
        ws.append(act_detail)

    ws.append([])
    ws.append(heading[4])
    accessories_.insert(0, all_keys_accessories) # key
    for acc_ in accessories_: #values
        ws.append(acc_)
        
    path = "projectMaster.xlsx"
    wb.save(path)
